...
window.BSU_DATA = {
  ...
  "scaling": {
    "language_encoding": {
      ...
      "scores": [0.251, 0.564, 0.873, 0.924, 0.997],
      "null_floor": 0.251,
      "reading": "How well each model predicts the brain's language network. Each sentence is read in the context of the passage it came from, the way a person reading it would have it. Untrained features already score 0.25, because the passage context by itself carries structure the brain follows; that is the null baseline on the left, and it is high for exactly that reason. The real models clear it by two to four times. One honest note about the right-hand end: BLIP-2 at 0.997 has essentially reached the noise ceiling, which is the most any model could score given how much the brain recordings themselves vary. That is a statement about the test running out of room, not about BLIP-2 being perfect, and it means this benchmark can no longer tell the strongest models apart. An earlier version of this chart showed lower numbers (GPT-2 around 0.53-0.60). That was a bug on our side: models wired in one way were shown each sentence alone while models wired another way got the passage, so the two were never measured on the same task. They now agree, checked against the long-standing Brain-Score language benchmark, which gives 0.821 for a matched model."
    },
    "it_encoding": {
  ...
  "inputs": [
    ...
    {"type": "text", "score": "0.997", "scored_with": "BLIP-2 OPT-2.7B", "scale": "share of the noise ceiling (untrained floor 0.251); at the ceiling, so this test no longer separates the strongest models", "cortex": "cortex_text.png", "example": "a sentence", "benchmark": "Pereira2018.243sentences-linear — human language-network fMRI", "desc": "A written sentence. The model returns its response to each sentence, scored against the language network's."},
